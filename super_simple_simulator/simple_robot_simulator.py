...
while running:
    ...
    instructions = (
        "Control the robot with the arrow keys. Press 'C' to center, "
        "'9' to rotate 90 degrees, 'P' to print pose, 'S' to show path, "
        "'R' to Reset path, 'T' to draw a triangle, 'Q' to quit."
    )
    text_i = font.render(instructions, True, "gray")
    screen.blit(text_i, (25, 10))

    if print_pos:
        # Print the robot position and orientation at the bottom of the screen
        text_p = font.render(
            f"Robot position: {int(robot_pos.x), int(robot_pos.y)} pixels; "
            f"orientation: {int(orientation * 180 / math.pi)} degrees.",
            True, "black"
        )
        screen.blit(text_p, (25, screen.get_height() - font_size - 10))
